<form method="POST" action="{{ $action }}" style="max-width:520px">
    ...
    <div style="margin-bottom:14px">
        <label for="codigo" style="display:block;font-size:12px;font-weight:500;color:var(--dash-text-secondary);margin-bottom:5px">{{ __('Código') }}<span style="color:var(--dash-clinical-crit-text)">*</span></label>
        <input id="codigo" name="codigo" type="text" value="{{ old('codigo', $causaExterna?->codigo ?? '') }}" required
            ...
            maxlength="8" autocomplete="off" placeholder="{{ __('Según tabla vigente') }}">
        @error('codigo')
    ...
    <div style="margin-bottom:18px">
        ...
        <input id="nombre" name="nombre" type="text" value="{{ old('nombre', $causaExterna?->nombre ?? '') }}" required
